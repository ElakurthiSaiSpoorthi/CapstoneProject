...
public class AccountSummaryStepDefinition {
	...
	static {
		try {
			...
			sparkReport.loadXMLConfig("src/test/resources/extent-config.xml");
	...
	public void user_should_see_the_following_account_types_displayed(DataTable accountTypesTable) {
		List<String> expectedTypes = accountTypesTable.asList();
	    List<String> actualTypes = accountSummaryPage.getDisplayedAccountTypes();

	    for (String expected : expectedTypes) {
	        Assert.assertTrue(actualTypes.contains(expected), "Missing account type: " + expected);
	    }}
